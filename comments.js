...
(function () {
  ...
  var CFG = {
    ...
    REPORT_THRESHOLD: 3,
    CLICK_HOOK: true   // 캔버스 노드 클릭 시 앱 상세패널에 토론 버튼 끼워넣기(콘텐츠 기반 감지)
  };

  var DATA = null, groupById = {}, nodeById = {}, labelToNode = {};
  var notedNodes = [];                 // note 있는 노드(클릭 훅 감지용)
  ...
  function init(data) {
    ...
    (data.nodes || []).forEach(function (n) {
      nodeById[n.id] = n; labelToNode[n.label] = n;
      if (n.note && n.note.length >= 12) notedNodes.push(n);
    });
    buildUI();
    loadRecaptcha();
    if (CFG.CLICK_HOOK) startClickHook();
  }

  /* ----------------------------- UI 구성 ----------------------------- */
  function buildUI() {
    var launcher = h('button', { id: 'gag-launcher', title: '노드별 토론' },
      h('span', { class: 'gag-ic' }, '💬'), document.createTextNode('토론'));
    launcher.addEventListener('click', function () { openPanel(); showDirectory(); });
    ...
    els = { launcher: launcher, overlay: overlay, panel: panel, head: head, back: back, title: title, body: body };
  }
  ...
  function showDirectory() {
    ...
    renderList(listHost, '');
    input.addEventListener('input', function () { renderList(listHost, input.value.trim().toLowerCase()); });
  ...
  function renderList(host, q) {
    ...
    groups.forEach(function (g) {
      var items = (DATA.nodes || []).filter(function (n) {
        return n.group === g.id && (!q || n.label.toLowerCase().indexOf(q) >= 0 || (n.note || '').toLowerCase().indexOf(q) >= 0);
      });
    ...
    if (!total) host.append(h('div', { class: 'gag-empty' }, '검색 결과가 없습니다.'));
  }
  ...
  function openNode(nodeId) {
    ...
    els.body.innerHTML = '';
    var g = groupById[node.group] || { label: '', };
    var color = hsl(g);
  ...
  function refresh(nodeId, countEl, listEl) {
    apiList(nodeId).then(function (res) {
      ...
      if (currentNodeId !== nodeId) return;
      countEl.textContent = '댓글을 불러오지 못했습니다.';
      listEl.innerHTML = '';
    });
  }
  ...
  function buildForm(nodeId, parentId, onDone, isReply) {
    ...
    submit.addEventListener('click', function () {
      ...
      recaptcha('comment').then(function (token) {
        ...
        if (res && res.ok) { ta.value = ''; msg.className = 'gag-msg ok'; msg.textContent = '등록되었습니다.'; if (isReply) form.remove(); onDone(); }
  ...
  function startClickHook() {
    var raf = null, lastId = null;
    var obs = new MutationObserver(function () { if (raf) return; raf = requestAnimationFrame(function () { raf = null; scan(); }); });
    obs.observe(document.body, { childList: true, subtree: true, characterData: true });

    function scan() {
      try {
        if (els.panel && els.panel.classList.contains('open')) return; // 내 패널 열려있으면 패스
        var panels = [];
        document.querySelectorAll('div').forEach(function (el) {
          if (el.id && el.id.indexOf('gag-') === 0) return;
          if (el.closest('#gag-panel') || el.closest('#gag-launcher')) return;
          var cs = el.ownerDocument.defaultView.getComputedStyle(el);
          if (cs.position !== 'fixed' && cs.position !== 'absolute') return;
          var t = (el.innerText || '').trim();
          if (t.length < 12 || t.length > 600) return;
          panels.push({ el: el, t: t });
        });
        var found = null;
        for (var i = 0; i < panels.length && !found; i++) {
          var t = panels[i].t;
          for (var j = 0; j < notedNodes.length; j++) {
            if (t.indexOf(notedNodes[j].note.slice(0, 24)) >= 0) { found = { node: notedNodes[j], host: panels[i].el }; break; }
          }
        }
        if (!found) { lastId = null; return; }
        if (found.node.id === lastId && document.getElementById('gag-hook-btn')) return;
        lastId = found.node.id;
        var old = document.getElementById('gag-hook-btn'); if (old) old.remove();
        var btn = h('button', { id: 'gag-hook-btn', class: 'gag-hook-btn' }, h('span', {}, '💬'), document.createTextNode(' 이 노드 토론 보기'));
        var nodeId = found.node.id;
        btn.addEventListener('click', function (e) { e.stopPropagation(); openNode(nodeId); });
        found.host.appendChild(btn);
      } catch (e) { /* 안전: 훅 실패해도 디렉터리는 동작 */ }
    }
  }
  ...
  function apiPost(payload) {
    // text/plain → preflight 회피. 응답을 못 읽어도 성공일 수 있으니 list 재조회로 보강.
